...
export async function scrapingVuelos(params) {
    const { origenIda, origenVuelta, departureDate, returnDate, adults, children, infants, stops, checkedBaggage, horarioIdaEntre, horarioIdaHasta, horarioVueltaEntre, horarioVueltaHasta, maxDuracionIda, maxDuracionVuelta, carryon, bodega, context, } = params;
    ...
    try {
        ...
        const salidaInput = page.getByRole('textbox', { name: '24SEP' }); // Este placeholder puede cambiar, verifica el HTML
        ...
        if (carryon) {
            ...
            for (const fila of filas) {
                ...
                if ((textoLabel === null || textoLabel === void 0 ? void 0 : textoLabel.trim()) === 'Con CarryOn') {
        ...
        if (bodega) {
            ...
            for (const fila of filas) {
                const label = fila.locator('label');
                const textoLabel = await label.textContent();
                if ((textoLabel === null || textoLabel === void 0 ? void 0 : textoLabel.trim()) === 'Con equipaje en bodega') {
                    // Ir al hermano: <div class="rz-chkbox-box"> dentro de <div class="rz-chkbox">
                    const box = fila.locator('.rz-chkbox-box');
                    await box.click();
                }
